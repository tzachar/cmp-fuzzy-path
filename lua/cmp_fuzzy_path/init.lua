...
source.get_keyword_pattern = function(_, params)
  if vim.api.nvim_get_mode().mode == 'c' then
    ...
    return params.option.path_regex or defaults.path_regex
  end
end
...
local function find_pattern(cursor_before_line, params)
  local match_start, match_end = regex(params.option.path_regex):match_str(cursor_before_line)
...
source.complete = function(self, params, callback)
  ...
  if is_cmd then
    ...
    if regex(params.option.cmd_trigger_regex):match_str(params.context.cursor_before_line) then
      pattern = params.context.cursor_before_line:sub(params.offset)
    else
      pattern = find_pattern(params.context.cursor_before_line, params)
    end
  else
    pattern = find_pattern(params.context.cursor_before_line, params)
  end
